...
app.post('/api/courses', (req, res) => {
  ...
  db.run(insertQuery, [courseName, courseNumber, section, term, startDate, endDate, instructorId], function (err) {
    if (err) {
      console.error(err.message);
...
app.get('/api/instructor/all', (req, res) => {
  db.all('SELECT UserID, FirstName, LastName, Email FROM tblUsers WHERE Role = ?', ['instructor'], (err, rows) => {
    if (err) {
      console.error(err.message);
